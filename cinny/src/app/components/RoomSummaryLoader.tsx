...
export function RoomSummaryLoader({ roomIdOrAlias, children }: RoomSummaryLoaderProps) {
  ...
  const fetchSummary = useCallback(async () => {
    try {
      return await mx.getRoomSummary(roomIdOrAlias);
    } catch (error) {
      // Nheko summary API not supported, return null to allow fallback
      console.debug('Room summary API not available, using fallback');
      return null;
    }
  }, [mx, roomIdOrAlias]);
